...
def download_tile(wms_service, layer_name, longitude, latitude, width_in_pixels, dop_resolution, image_format):
    ...
    bounding_box = (longitude - width_from_center, 
                    ...
                    latidude + width_from_center)

    return wms.getmap(layers=[layer_name], styles=['default'], srs='EPSG:25832',
                bbox=bounding_box, 
                size=(width_in_pixels, width_in_pixels), format='image/jpeg', transparent=True)
...
create_dir(os.path.join(args["output"], "positives"))
create_dir(os.path.join(args["output"], "negatives"))
...
with open(args["input"], "r") as csv_file:
    ...
    for i, row in enumerate(csv_reader):
        longitude = float(csv_reader["x"])
        latitude = float(csv_reader["y"])
        label = "positive" if bool(csv_reader["label"]) else "negative"
        
        output_path = os.path.join(os.path.join(args["output"], label))
        output_path = os.path.join(output_path, "{}.png".format(row["object_id"]))

        tile_image = download_tile(args["wms_service"], args["layer"], longitude, latitude, output_path, args["width"], args["dop"], args["image_format"])
